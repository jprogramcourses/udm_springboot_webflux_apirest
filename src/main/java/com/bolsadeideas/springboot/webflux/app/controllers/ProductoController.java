...
public class ProductoController {
	...
	public Mono<ResponseEntity<Producto>> ver(@PathVariable String id) {
		return service.findById(id).map(p -> ResponseEntity.ok(p)).defaultIfEmpty(ResponseEntity.notFound().build());
	}

	@PostMapping
	public Mono<ResponseEntity<Map<String, Object>>> crearProducto(@Valid @RequestBody Mono<Producto> monoProducto) {
	...
	@PostMapping("/v2")
	public Mono<ResponseEntity<Producto>> crearProductoConFoto(Producto producto, @RequestPart FilePart file) {
		if (producto.getCreateAt() == null) {
			producto.setCreateAt(new Date());
		}
	...
	@PutMapping("/{id}")
	public Mono<ResponseEntity<Producto>> actualizaProducto(@PathVariable String id, @RequestBody Producto producto) {
